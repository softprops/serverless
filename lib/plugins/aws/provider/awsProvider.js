...
class AwsProvider {
  ...
  /**
   * Get API Gateway Rest API Stage description.
   * Keys are expected in camelCase and will be exported in cloudformations PascalCase
   *
   * See https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-apigateway-deployment-stagedescription.html
   * for informatation on cloud formation support of StageDescriptions
   */
  getApiGatewayStageDescription() {
    if (this.serverless.service.provider.apiGateway
      && this.serverless.service.provider.apiGateway.stageDescription) {
      const stageDescription = this.serverless.service.provider.apiGateway.stageDescription;
      if (!_.isPlainObject(stageDescription)) {
        throw new Error('REST API stage description must be an object');
      }
      const tracingEnabled = stageDescription.tracingEnabled;
